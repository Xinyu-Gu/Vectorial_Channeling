...
model.param.set('k0', '0.0259*2'); %delta U=2kBT
model.param.set('r0', '45 [nm]'); 
model.param.set('w_theta', '5 [nm]'); 
...
model.component('comp1').physics('tds').feature('cdm1').set('V', 'k0/2*(1+tanh((r0-sqrt(x^2+y^2+z^2))/w_theta)) [V]');
...
model.component('comp1').physics('tds').feature('cdm2').set('z', 1);
model.component('comp1').physics('tds').feature('cdm2').set('V', 'k0/2*(1+tanh((r0-sqrt(x^2+y^2+z^2))/w_theta)) [V]');
